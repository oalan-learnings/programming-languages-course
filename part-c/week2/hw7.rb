...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if(in_between(@x, seg.x1, seg.x2) and in_between(@y, seg.y1, seg.y1))
      Point.new(x,y)
    else
      NoPoints.new
    end
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close_point(@x1, @y1, @x2, @y2)
      Point.new(@x1, @y1)
    elsif ((@x1 > @x2) and (not (real_close(@x1,@x2)))) or (real_close(@x1,@x2) and (@y1 > @y2))
...
class Let < GeometryExpression
  ...
  def eval_prog env
    @e2.eval_prog(env.clone.unshift([@s, @e1.eval_prog(env)]))
  end
  def preprocess_prog
